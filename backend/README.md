...
| `GET` | `/rating/history?user_id=...` | query: `user_id` | `{"ratings": [{"date": string, "rating": string}]}` | 直近30日分の rating 履歴を返す。各日の rating はその日以前の最新回答時点までの全回答ログから計算し、今日と直近30日の最初の日は必ず返す。それ以外の日は、その日に回答ログがある日だけ返す。ユーザーが存在しない場合は `404` |
