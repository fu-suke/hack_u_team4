...
3. `Timer seconds` に秒数、`Commands` に検出したい文字列を入れます。デフォルトは `/focus` です。
4. `+` で Command 入力欄を追加できます。
